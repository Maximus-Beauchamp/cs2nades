  ...
  if (image) {
    ...
    img.onload = () => {
      ...
      function getRandomPosition() {
        const x = Math.floor(Math.random() * (canvasWidth - img.width));
        const y = Math.floor(Math.random() * (canvasHeight - img.height));
        return { x, y };
      }
  
      // Draw the image at a random position on the canvas
